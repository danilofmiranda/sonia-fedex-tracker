...
def generate_sonia_analysis(track_data, status, is_delivered, delivery_date, ship_date, label_date):
    ...
    if is_delivered:
        if delivery_date and ship_date:
            try:
                ...
                if transit_days <= 2:
                    ...
                    recommendation = "Delivery took longer than usual."
            except:
        ...
        if label_date:
            try:
                ...
                if days_since_label > 5:
                    recommendation = f"ATTENTION: {days_since_label} days since label created. Contact shipper."
                elif days_since_label > 2:
                    recommendation = "Label created but not yet picked up."
                else:
        ...
        if ship_date:
            try:
                ...
                if days_in_transit > 7:
                    recommendation = f"ATTENTION: {days_in_transit} days in transit. Check for delays."
                elif days_in_transit > 4:
                    recommendation = "Extended transit time. May be in customs."
                else:
        ...
        recommendation = "Package out for delivery today!"
    elif "exception" in status.lower() or "hold" in status.lower():
        recommendation = "ACTION REQUIRED: Package has an exception."
    elif "customs" in status.lower() or "clearance" in status.lower():
        recommendation = "Package in customs clearance."
    else:
            ...
            if complete_track:
                ...
                if track_result:
                    ...
                    result["sonia_analysis"] = generate_sonia_analysis(
                        track_data, result["status"], result["is_delivered"],
...
async def process_file(file: UploadFile = File(...)):
    try:
        ...
        if tracking_col is None:
            return JSONResponse({"success": False, "error": "No se encontro la columna HAWB con numeros de tracking"})
        logger.info(f"Using tracking column: {tracking_col}, client column: {client_col}")
        client = FedExClient()
        results = []
        ...
        if not results:
            return JSONResponse({"success": False, "error": "No se encontraron numeros de tracking validos en la columna HAWB"})
        logger.info(f"Processed {len(results)} tracking numbers successfully")
        output_df = pd.DataFrame(results)
        output_df = output_df[[
            "client_name", "tracking_number", "status", "sonia_analysis",
            "label_creation_date", "ship_date", "days_after_shipment",
            "working_days_after_shipment", "days_after_label_creation", "destination_location"
        ]]
        output_df.columns = [
            "Nombre Cliente", "FEDEX Tracking", "Status", "SonIA",
            "Label Creation Date", "Shipping Date", "Days After Shipment",
            "Working Days After Shipment", "Days After Label Creation", "Destination City/State/Country"
        ]
        output = BytesIO()
